...
12. Never use "psychiatric", "mental", "self-harm", "suicide", "kill",
    "murder", or similar real-world harm language as a joke.
...
def article_to_pdf(
    ...
    deck_style = ParagraphStyle(
        ...
        fontSize=11,
        leading=14,
        textColor=colors.HexColor("#4e596b"),
        spaceAfter=10,
    )
    ...
    story.append(
        Paragraph(
            f"GAMEWEEK {safe_int(gw)} • {html.escape(str(league_name))}",
            subtitle
        )
    )
    ...
    # Pull the AI's internal sections out and give them proper editorial labels.
    def section(blob, key):
        pattern = rf"(?ms)^#\s*{re.escape(key)}\s*$\n(.*?)(?=^#\s|\Z)"
        match = re.search(pattern, blob)
        return match.group(1).strip() if match else ""

    headline = section(article, "HEADLINE")
    deck = section(article, "DECK")
    front = section(article, "FRONT_PAGE_BLURB")
    line = section(article, "LINE_OF_THE_WEEK")
    signoff = section(article, "SIGN_OFF")

    # Clean accidental internal labels/metadata if an older model emitted them.
    def clean_export_body(value):
        value = re.sub(
            r"(?im)^\s*(ROAST_[1-5]|FRONT_PAGE_BLURB|KING_VISUAL|"
            r"DISASTER_VISUAL|CAPTAIN_VISUAL|BENCH_VISUAL|LEAGUE_GOSSIP|"
            r"TITLE_RACE|WOODEN_SPOON|LINE_OF_THE_WEEK|SIGN_OFF)\s*$",
            "",
            value,
        )
        value = re.sub(
            r"(?im)^\s*(Team|League Position)\s*:\s*.*$",
            "",
            value,
        )
        value = re.sub(
    ...
    headline = clean_export_body(headline)
    deck = clean_export_body(deck)
    front = clean_export_body(front)
    line = clean_export_body(line)
    signoff = clean_export_body(signoff)
    ...
    for label, key in roast_sections:
        body = clean_export_body(section(article, key))
        if not body:
            continue

        # Keep gossip as a compact block; keep roasts as newspaper columns.
        story.append(
            Paragraph(
                html.escape(label),
                section_style
...
def article_to_txt(
    ...
    labels = [
        ...
        ("DECK", "THE DECK"),
        ("FRONT_PAGE_BLURB", "FRONT PAGE"),
        ("ROAST_1", "KING OF THE WEEK"),
        ("ROAST_2", "DISASTERCLASS"),
        ("ROAST_3", "CAPTAINCY CRIMINAL"),
        ("ROAST_4", "BENCH CRIME"),
        ("ROAST_5", "ANOTHER WEEKLY CASUALTY"),
        ("LEAGUE_GOSSIP", "LEAGUE GOSSIP"),
        ("TITLE_RACE", "TITLE RACE"),
        ("WOODEN_SPOON", "WOODEN SPOON"),
        ("LINE_OF_THE_WEEK", "LINE OF THE WEEK"),
        ("SIGN_OFF", "SIGN-OFF"),
    ]
    ...
    chunks = [
        "THE MINI-LEAGUE TIMES",
        f"Gameweek {safe_int(gw)} — {league_name}",
        "=" * 60,
        "",
    ]

    for key, label in labels:
        body = clean(section(article, key))
        if body:
            chunks.extend([
                label,
                "-" * len(label),
